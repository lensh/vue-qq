export default class Chat {
	...
	static async getMessage(userId, otherUserId) {
		...
		let sql=`
			UPDATE chat SET is_enter_chat =1
        ...
        //用户信息
		sql=`
			SELECT a.device,b.beizhu,c.face
			FROM user a, friend b,user_detail c
        ...
        //别人对自己的备注（socket时会用到）
		sql=`
			SELECT beizhu from friend 
			WHERE user_id = ? AND other_user_id = ? limit 1
		`
	...
	static async updateEnterChat(userId, otherUserId) {
		const sql = `
			UPDATE chat SET is_enter_chat =0
			WHERE user_id = ? AND other_user_id = ?
		`
	...
	static async getStatus(userId, otherUserId) {
		const sql = `select is_pingbi,is_enter_chat from chat where user_id = ? 
			AND other_user_id = ? limit 1`
