...
import { get_all_message } from '@/api/message'
...
const actions = {
  getAllMessage: ({commit},userId) => {
      get_all_message(userId).then((res)=>{
          ...
          for(let [index,value] of allMessage.entries()){
            if(value.message.length>20){  //截取20个字符
             allMessage[index]['message']=`${value.message.substr(0,20)}...`
            }
